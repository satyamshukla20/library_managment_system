...
def member_delete(id):
    ...
    if member.debt<-500:
        return {"message":f"{member.name} with {id} as id has current debt of {abs(member.debt)-500} please pay the remaining amount."}
    member.delete(id)
    return {"message":f"{member.name} with {id} as id is deleted now you are no longer member of the library."}
